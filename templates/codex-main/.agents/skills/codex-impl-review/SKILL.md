...
8. runner の `VERDICT:` と `Cycle:` を確認する。
9. `VERDICT: APPROVED` なら終了する。
10. `CONDITIONAL` / `REVISE` なら P0/P1 を優先して修正し、ユーザーへ戻らず即 rerun する。
11. 同種失敗が 3 回続く場合は `.agents/context/failure_report.md` に切り出す。
12. runner は `.agents/reviews/sessions.json` に cycle 数を残すので、停止判断にはそれも使う。
