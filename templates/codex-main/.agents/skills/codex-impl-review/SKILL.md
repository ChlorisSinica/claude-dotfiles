...
7. まず `{{PYTHON_LAUNCHER}} scripts/run-codex-impl-review.py` を使う。ad hoc な長い 1 行コマンドや一時 `_tmp` スクリプトは避ける。長時間 review で待ち時間を延ばしたい場合だけ `--review-timeout-sec <seconds>` を付ける。既定は 600 秒。
