...
Phase 2a:  /plan <機能の説明>            ← plan.md + tasks.md 作成（Claude が設計）
...
| `/plan` | 2a | plan.md + tasks.md 作成（Discussion Points を含む） | 機能の説明（**必須**） |
| `/sonnet-dp-research` | 2b | Discussion Points を Sonnet subagent で外部技術調査 | 追加コンテキスト（省略可） |
...
| `.claude/agents/sonnet-dp-research.md` | Discussion Points 外部調査 subagent |
| `.claude/agents/opus-review-validator.md` | Codex レビュー出力検証 subagent（オプション） |
| `.claude/agents/workflows/handover.md` | Handover ワークフロー定義 |
| `.claude/agents/workflows/retro.md` | Retro ワークフロー定義 |
| `.claude/agents/sessions.json` | レビュー監査ログ（append-only） |
