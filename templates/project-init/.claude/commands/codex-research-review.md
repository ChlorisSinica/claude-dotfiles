...
- Claude が `.claude/context/research.md` を既に作成済みであること

## 手順

1. `.claude/context/research.md` を読み込み、内容を確認

2. `.claude/agents/prompts/codex_research_review.md` のプロンプトテンプレートを読み込む

3. テンプレート内の `$FEATURE` を置換（`$ARGUMENTS` が空なら research.md の概要から推定）

4. プロンプトと research.md を結合して Codex に送信:
```powershell
$prompt = Get-Content ".claude/agents/prompts/codex_research_review.md" -Raw
$prompt = $prompt -replace '\$FEATURE', '$ARGUMENTS'
$research = Get-Content ".claude/context/research.md" -Raw
"$prompt`n`n---`n`n# research.md`n$research" | codex review -
```

5. Codex のレビュー結果に基づき `.claude/context/research.md` を更新
