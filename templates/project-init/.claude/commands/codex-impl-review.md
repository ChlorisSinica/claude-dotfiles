...
2. `.agents/prompts/codex_impl_review.md` のプロンプトテンプレートを読み込む

3. テンプレートのプレースホルダを置換:
   - `$TASK_DESCRIPTION` → タスク説明
   - `$FILE_LIST` → 対象ファイルのパス一覧

4. 対象ファイルと依存ファイルの内容を収集・結合し `.context/_codex_input.tmp` に書き出す

5. **サイクル2以降: 前回指摘の注入**
   - `.context/codex_impl_review.md`（前回の Codex 出力）が存在し、かつサイクル1でない場合:
...
7. **Codex 出力を `.context/codex_impl_review.md` に保存する**（毎サイクル上書き）
...
9. APPROVED 取得後、`.agents/sessions.json` の `last_impl_review` キーを更新

## 終了条件

1. Codex が **APPROVED** を返した → サイクル終了
2. **5サイクル到達** → ユーザーに状況報告し判断を仰ぐ（これが唯一のユーザー確認ポイント）
3. **同種失敗が3回**続いた → 停止し `.context/failure_report.md` に詳細を記録
...
2. 変更ファイルを `git add`（`.context/` 配下も含む）
