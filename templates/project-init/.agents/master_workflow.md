...
**（開発）**
`.agents` と `.context` と `.claude` 以外のこのプロジェクトのファイルを全てじっくり読み，仕組みや役割，そしてすべての特徴を深く理解してください．それが終わったら，学んだことや発見や知っていることを全て詳細にまとめ，`.context/research.md` を日本語で作成してください．

**（不具合）**
タスクのスケジューリングフローをよく理解し，潜在的なバグを探してください．すべてのバグを見つけるまでフローの調査を続けてください。終わったら，調査結果の詳細な報告書を `research.md` に専用のセクションとして追加してください．

**（更新）**
既に opus 4.6 が作成した `.context/research.md` が存在する場合は削除せず，レビューも行い，不足分や差分を記述して更新してください．
...
```
LOOP（未完了タスクがなくなるまで）:
  0) .context/tasks.md を順番に follow する
  1) 1タスクのみ最小変更で実装
  ...
  4) PASS したら [x] 更新
  5) 同種失敗が3回続いたら停止して .context/failure_report.md に報告
END LOOP
