...
`/init-project`（Claude）/ `$init-project`（Codex）/ Python 直接呼び出しの 3 つの呼び出し方に共通する `init-project.py` の仕様と運用です．

## smart mode

`/init-project` は manifest（`.claude-dotfiles-managed.json`）の有無で動作モードを自動判定します．
...
1. `/init-project <preset> --fresh` で全上書き（`context/` / `reviews/` / `logs/` は保持される）
2. または衝突しているファイルを手で退避・削除してから `/init-project <preset>` を再実行
...
```text
/init-project           # manifest から preset / template を復元して更新
$init-project           # 同上
```

## 呼び出し方の使い分け

- **Claude Code の `/init-project`**: 3 つの template すべてに対応
- **Codex の `$init-project`**: `codex-main` 専用．他 template は Python か Claude Code から
- **Python 直接呼び出し**: どの環境からでも使える．実体はこれ
