...
- `-t codex-main` は `.agents/skills/` と `.agents/context/` を主軸としたテンプレートを生成し，Python review runner `scripts/run-codex-*.py`，`scripts/run-verify.py`，Codex ランタイム設定 `.claude/settings.json` / `.claude/settings.local.json(.bak)` を出力する
