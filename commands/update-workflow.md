...
- `--codex-main --workflow-only -f` は `.agents/context/` と `.agents/reviews/` を保持しつつ、`.agents/skills/`、prompt/template 資産、`.claude/settings.json`、`.claude/settings.local.json(.bak)`、`scripts/run-verify.*`、`scripts/run-codex-*-review.ps1`、`.gitignore` を更新する
