...
## `init-project.py` の呼び出し方

プロジェクト雛形の作成と更新の本体は `~/.claude/scripts/init-project.py` です．Claude の `/init-project` と Codex の `$init-project` はこの Python スクリプトへのショートカットです．
...
- `codex-main`: Codex 主体．`.agents/AGENTS.md`，`.agents/{skills,prompts,context,reviews,scripts}/`，review runner（`.agents/scripts/run-codex-*.py`）を生成
- `research-survey`: `survey-*` preset 用．`init-project.py` で研究用 command 群と `survey-convert.py` を生成

追加で必要なツールは用途別:

- **雛形の作成のみ**: Python 3.11+ だけで足ります
- **`codex-main` の review runner や Claude 側の `/codex:*` 系コマンド**: Codex CLI（`npm install -g @openai/codex`）
- **`research-survey` の `survey-convert.py`**: Pandoc（`winget install --id JohnMacFarlane.Pandoc`）
- **生成された `.claude/scripts/run-verify.py` の実行**: preset 依存．TypeScript なら Node，Rust なら Cargo，AutoHotkey なら AHK，C++ / Unity なら MSVC．一覧は [Init Project](./docs/init-project.md)

## `init-project.py` の最小理解
...
- [Init Project](./docs/init-project.md) — `init-project.py` の仕様，モード，エラー対処，preset 別の検証ツール
