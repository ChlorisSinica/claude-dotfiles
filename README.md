...
`https://nyosegawa.com/posts/claude-code-statusline-rate-limits/` のPattern5を拡張した2行構成のカスタムステータスライン:
...
ステータスラインのみをセットアップする場合:

```bash
bash ~/claude-dotfiles/setup-statusline.sh      # 新規インストール
bash ~/claude-dotfiles/setup-statusline.sh -f    # 上書き更新
```

`setup.sh` を実行した場合も自動でセットアップされます。
